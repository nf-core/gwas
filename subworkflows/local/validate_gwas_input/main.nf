...
def validateRelationalInput(cohort_rows, analysis_rows, summary_statistics_rows, relationship_rows, cohort_manifest, analysis_manifest, summary_statistics_manifest, relationship_manifest, cohort_schema, analysis_schema, summary_statistics_schema, relationship_schema, reference_catalog, method_options = null) {
    ...
    def pair_prevalence_analysis_ids = relationship_rows.findAll { row -> tokenizeMethodSelector(row[0].relationship_methods).contains('gcta_bivariate_reml') }.collectMany { row -> [normaliseCellValue(row[0].left_analysis_id), normaliseCellValue(row[0].right_analysis_id)] }.findAll { analysis_id -> analysis_id }.collect { analysis_id -> analysis_id.toString() } as Set
